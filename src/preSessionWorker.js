...
function toApiShape(row) {
  ...
  return {
    ...
    appPracticeSinceLastClass: {
      ...
      overallAccuracy: Number(briefJson?.appPracticeSinceLastClass?.overallAccuracy || 0),
      topGameTypes: briefJson?.appPracticeSinceLastClass?.topGameTypes || [],
      grammarMistakeCount: Number(briefJson?.appPracticeSinceLastClass?.grammarMistakeCount || 0),
      pronunciationMistakeCount: Number(briefJson?.appPracticeSinceLastClass?.pronunciationMistakeCount || 0),
      lastActivityAt: briefJson?.appPracticeSinceLastClass?.lastActivityAt || null,
    },
    ...
    readinessNumeric: Number(briefJson?.readinessNumeric || 0),
    readinessRationale: String(briefJson?.readinessRationale || '').trim() || null,
    confidence: String(briefJson?.confidence || 'low').toUpperCase(),
    classContext: briefJson?.classContext || row.lastSessionSummary?.classContext || null,
    qualityDiagnostics: briefJson?.qualityDiagnostics || null,
